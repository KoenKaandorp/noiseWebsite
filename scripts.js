...
var FADE_TIME = .25;
const FADE_SET_TARGET_TIME_CONSTANT = .05
  , MIN_FREQ = 1
  , MAX_FREQ = 20154
  , MIN_PIANO_KEY = 1
  , MAX_PIANO_KEY = 99
...
var BrowserDetect = {
    init: function() {
        ...
        if ("Explorer" == this.browser) {
            if (this.IE = !0,
            8 >= this.majorVersion || 8 === document.documentMode)
                this.IE8 = !0
        } else
            "Firefox" == this.browser ? this.Firefox = !0 : "Chrome" == this.browser ? this.Chrome = !0 : "Safari" == this.browser ? this.Safari = !0 : "Edge" == this.browser ? this.Edge = !0 : "Opera" == this.browser && (this.Opera = !0);
    ...
    identifyBrowser: function() {
        for (var a = 0; a < this.dataBrowser.length; a++) {
            ...
            if (-1 != (c.string || navigator.userAgent).indexOf(c.subString)) {
                ...
                (c = this.searchVersion(navigator.userAgent, a)) ? (this.version = c,
                this.majorVersion = Math.floor(c)) : (c = this.searchVersion(navigator.appVersion, a)) ? (this.version = c,
                this.majorVersion = Math.floor(c)) : this.majorVersion = this.version = "Unknown";
                return
            }
        }
...
function writeCookieMessage(a, c, b) {
    getCookie("cOK") || (window.onCookieConsent = function() {
    ...
    document.write("<div id=cookie-message>" + c + " <a href='" + a + "'>Learn more</a> <a href='javascript:void(0)' onclick='onCookieConsent()'>Got it</a></div>"))
}
function DocumentClickTracker() {
    ...
    a.onTouchStart = function(c) {
        1 != c.touches.length || c.altKey || c.shiftKey || c.ctrlKey || c.metaKey ? null !== a.touchStartData.id && (a.touchStartData.id = null) : (a.touchStartData.id = c.changedTouches[0].identifier,
    ...
    a.onTouchEnd = function(c) {
        0 != c.touches.length || c.altKey || c.shiftKey || c.ctrlKey || c.metaKey || c.changedTouches[0].identifier != a.touchStartData.id || a.ignoredElement && a.ignoredElement.contains(c.target) || (a.callback(a.touchStartData.target),
        a.touchStartData.id = null)
    }
    ;
...
function AdUnit(a, c, b) {
    ...
    d.blocked = !1;
    d.disable = function() {
        d.removed || d.blocked || (g ? g.style.visibility = "visible" : (g = document.createElement("div"),
        g.className = "ad-banner-blocker",
        g.style.visibility = "visible",
        g.addEventListener("touchend", function(h) {
            h.preventDefault()
        }, !1),
        f.appendChild(g)))
    }
    ...
    d.remove = function(h) {
        ...
        function m(n) {
            "height" == n.propertyName && (e.classList.add("removed"),
            b && b(),
        ...
        void 0 !== h && (p = -1 < p ? d.id.slice(0, p) : d.id,
...
function onPlayButtonClick() {
    tones.playing ? (window.playButton.innerHTML = "Play",
    tones.stop()) : (window.playButton.innerHTML = "Stop",
...
function formatNumber(a) {
    a = a.toString().split(".");
    if (1 == a.length)
        return a[0].toLocaleString("en-US", {
            style: "currency",
            useGrouping: !0
        }) + " <small>Hz</small>";
    if (2 == a.length)
        return 2 >= a[1].length ? a[0].toLocaleString("en-US") + "<small>." + a[1] + " Hz</small>" : "<small>~&thinsp;</small>" + a[0].toLocaleString("en-US") + "<small>." + a[1].slice(0, 2) + " Hz</small>"
}
...
function formatHertz(a) {
    a = a.toString().split(".");
    if (1 == a.length)
        return separateThousands(a[0]) + " <small>Hz</small>";
    if (2 == a.length) {
        if (2 >= a[1].length)
...
function setFreq(a) {
    if (a < MIN_FREQ || a > MAX_FREQ)
        return !1;
    if (a == window.sliderFreq)
        return !0;
...
function FrequencyReadout(a) {
    var c = this, b = document.querySelector(a), d, e;
    if (!b)
    ...
    c.update = function(k) {
        ...
        0 < window.sliderFreq - q ? window.sliderFreq.toString().length <= q.toString().length + k + 1 ? (k = q.toString(),
        g.nodeValue = separateThousands(k),
        h.nodeValue = window.sliderFreq.toString().slice(k.length),
        c.fractionOn = !0,
        f.nodeValue = "") : (q = Math.round(window.sliderFreq * Math.pow(10, k)) / Math.pow(10, k),
        g.nodeValue = separateThousands(Math.floor(q).toString()),
        h.nodeValue = q.toFixed(k).slice(-(k + 1)),
        c.fractionOn = !0,
        f.nodeValue = "~\u2009",
        c.tildeOn = !0) : (g.nodeValue = separateThousands(q.toString()),
...
function Dialog(a) {
    ...
    c.show = function(b, d, e) {
        ...
        if (d.fullscreen)
            if (b = c.div.getBoundingClientRect(),
            ...
            void 0 === d.fullscreenVertPoint || isNaN(d.fullscreenVertPoint))
                ...
                g = getViewportHeight();
                const h = Math.min(b.height / 2 / g, .5);
                f.top = Math.round(g * Math.min(Math.max(d.fullscreenVertPoint, h), 1 - h) - b.height / 2) + "px"
            }
        reflow(c.div.clientHeight);
        d.closeOnDocumentClick && (c.tracker = new DocumentClickTracker,
...
var noteSelectorWindow = {
    ...
    show: function(a, c) {
        ...
        noteSelectorWindow.dialog.show && (null == noteSelectorWindow.panel && noteSelectorWindow.prepare(),
        noteSelectorWindow.dialog.show(noteSelectorWindow.panel, {
            closeButton: !0
    ...
    highlightButton: function(a) {
        noteSelectorWindow.selectedButton && noteSelectorWindow.selectedButton.classList.remove("selected");
        -1 != a && (a = noteSelectorWindow.buttonArray[a]) && (a.classList.add("selected"),
        noteSelectorWindow.selectedButton = a)
    }
};
function NoteSelector(a, c) {
    ...
    b.updateFromFreq = function() {
        var d = 12 * Math.log2(window.sliderFreq / 440) + 49
          , e = Math.round(d);
        e < MIN_PIANO_KEY ? (b.buttonText.nodeValue = "below A0",
        b.tilde.nodeValue = "") : e > MAX_PIANO_KEY ? (b.buttonText.nodeValue = "above " + NOTE_NAMES[MAX_PIANO_KEY],
        b.tilde.nodeValue = "") : (b.buttonText.nodeValue = NOTE_NAMES[e],
...
function DropDownMenu(a) {
    ...
    c.show = function(b, d, e, f) {
        for (var g = document.createDocumentFragment(), h = 0; h < b.length; h++) {
            ...
            l.value == d && (l.classList.add("selected"),
            c.selectedButton = l);
            g.appendChild(l)
        }
        c.callback = f;
...
function WaveSelector(a, c) {
    ...
    b.onMenuOutput = function(g) {
        g === DropDown.DROPDOWN_CLOSE ? (b.button.classList.remove("control-shown"),
        b.menuShown = !1) : c(g)
    }
    ;
    b.update = function() {
    ...
    b.button.onclick = function(g) {
        ...
        g.show && (g.show(e, b.value, this, b.onMenuOutput),
...
function BalanceControl(a, c) {
    ...
    b.onDropDownAction = function(d) {
        d === DropDown.DROPDOWN_CLOSE ? (b.button.classList.remove("control-shown"),
        b.shown = !1) : c(d)
    }
    ;
    b.update = function(d) {
        ...
        b.LspeakerIcon && (b.LspeakerIcon.style.opacity = Math.round(100 * e) / 100);
        b.RspeakerIcon && (b.RspeakerIcon.style.opacity = Math.round(100 * f) / 100);
        b.shown && (b.readoutLeftVal.nodeValue = Math.round(100 * e) + "%",
    ...
    b.button.onclick = function(d) {
        ...
        f.setAttributeNS("http://www.w3.org/1999/xlink", "href", "#svg-Lspeaker-icon");
        e.appendChild(f);
        d.appendChild(e);
        ...
        f = document.createElementNS("http://www.w3.org/2000/svg", "use");
        f.setAttributeNS("http://www.w3.org/1999/xlink", "href", "#svg-Rspeaker-icon");
        e.appendChild(f);
        d.appendChild(e);
...
function handleKeyDown(a) {
    if (!a.altKey && !a.metaKey)
        switch (a.keyCode) {
        case 37:
            if (a.target != document.body && "BUTTON" != a.target.nodeName && a.target.parentNode != window.slider)
                break;
            a.preventDefault();
            a.stopPropagation();
...
function blockSpaceKeyUp(a) {
    a.ctrlKey || a.altKey || a.metaKey || 32 != a.keyCode || (a.preventDefault(),
    a.stopPropagation())
}
function UpDownButton(a, c) {
    ...
    this.button.onmousedown = function(d) {
        b.timeoutID || b.intervalID || (b.action(),
    ...
    this.button.ontouchstart = function(d) {
        b.timeoutID || b.intervalID || (d.preventDefault(),
        b.action(),
        b.timeoutID = setTimeout(b.startRepeatPress, 500))
    }
    ;
    ...
    this.button.onkeydown = function(d) {
        if (!(b.timeoutID || b.intervalID || d.shiftKey || d.ctrlKey || d.altKey || d.metaKey))
            switch (d.keyCode) {
            case 13:
                b.action(),
                d.preventDefault(),
    ...
    if ("" != a) {
        ...
        for (let h of a)
            if (/^\d+\.?\d*$/.test(h) && void 0 === e && void 0 === f)
                ...
                void 0 === c && /^v\d\.?\d*$/.test(h) ? c = parseFloat(h.substr(1)) : void 0 === b && /^b-?\d\.?\d*$/.test(h) && (b = parseFloat(h.substr(1)))
    }
    if (void 0 === e)
        e = void 0 !== f && -1 != f ? 440 * Math.pow(2, (f - 49) / 12) : 440;
    else if (e < MIN_FREQ || e > MAX_FREQ)
        e = 440;
    void 0 === g && (g = "sine");
    if (void 0 === c || 0 > c || 1 < c)
        c = DEFAULT_VOLUME;
...
function constructHash() {
    ...
    0 == a % 1 && a >= MIN_PIANO_KEY && a <= MAX_PIANO_KEY ? (a = /^\w\u266f?\d/.exec(NOTE_NAMES[a]),
    a = null !== a ? a[0].replace("\u266f", "s") : window.sliderFreq) : a = window.sliderFreq;
...
function Tone(a, c) {
    ...
    b.init = function(d, e, f, g) {
        ...
        2 == b.channels && (b.mergerNode = a.createChannelMerger(2),
        ...
        b.volume = f || 1;
        b.balance = g || 0;
        2 == b.channels && (b.leftGainNode.gain.value = Math.min(1, 1 - b.balance),
    ...
    b.initOscillator = function() {
        ...
        2 == b.channels ? (b.oscillator.connect(b.leftGainNode),
        b.oscillator.connect(b.rightGainNode)) : b.oscillator.connect(b.gainNode);
    ...
    b.play = function(d) {
        ...
        BrowserDetect.Firefox ? b.fadeGainNode.gain.setTargetAtTime(1, d, FADE_SET_TARGET_TIME_CONSTANT) : (b.fadeGainNode.gain.cancelScheduledValues(d),
        b.fadeGainNode.gain.setValueAtTime(b.fadeGainNode.gain.value, d),
        b.fadeGainNode.gain.linearRampToValueAtTime(1, d + FADE_TIME * (1 - b.fadeGainNode.gain.value)));
        b.playing = !0
    }
    ;
    b.stop = function(d) {
    ...
    b.setFreq = function(d) {
        b.freq = d;
        b.playing && b.oscillator.frequency.setTargetAtTime(d, a.currentTime, .03)
    }
    ;
    ...
    b.setBalance = function(d) {
        if (2 !== b.channels)
            return !1;
        b.balance = d;
        b.playing ? (b.leftGainNode.gain.setTargetAtTime(Math.min(1, 1 - d), a.currentTime, FADE_SET_TARGET_TIME_CONSTANT),
        b.rightGainNode.gain.setTargetAtTime(Math.min(1, 1 + d), a.currentTime, FADE_SET_TARGET_TIME_CONSTANT)) : (b.leftGainNode.gain.setValueAtTime(Math.min(1, 1 - d), a.currentTime),
        b.rightGainNode.gain.setValueAtTime(Math.min(1, 1 + d), a.currentTime))
    }
    ;
    b.setVolume = function(d) {
        b.playing ? b.gainNode.gain.setTargetAtTime(d, a.currentTime, FADE_SET_TARGET_TIME_CONSTANT) : b.gainNode.gain.setValueAtTime(d, a.currentTime);
        b.volume = d
    }
}
var tones = {
    ...
    init: function() {
        if (window.AudioContext)
            tones.context = new AudioContext;
        else if (window.webkitAudioContext)
            tones.context = new webkitAudioContext;
        else
            throw "Cannot initialize AudioContext";
    },
    add: function() {
    ...
    play: function() {
        ...
        for (var a = 0; a < tones.array.length; a++)
            tones.array[a].play()
    },
    stop: function() {
        tones.playing = !1;
...
function init() {
    ...
    sliderHandle && (sliderHandle.tabIndex = 20,
    ...
    window.getLinkButton.onclick = function() {
        ...
        navigator.clipboard.writeText(getShortURL()).then(function() {
            b.show(document.createTextNode("Link to current tone copied"), window.getLinkButton)
        }, function() {
            b.show(document.createTextNode("Error: Could not copy link to clipboard"), window.getLinkButton)
        })
    }
    ;
    tones.init();
